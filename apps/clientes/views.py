...
def catalogo_servicios(request):
    ...
    # Obtiene todos los servicios y combos
    servicios = Servicio.objects.all()
    combos = Combo.objects.all()

    # Filtra los servicios y combos si hay una consulta de búsqueda
    if query:
        servicios = servicios.filter(titulo__icontains=query)
        combos = combos.filter(nombre__icontains=query)

    # Filtra por tipo si se especifica (servicio o combo)
    if tipo == 'servicio':
    ...
    # Ordena los elementos por título o nombre
    ...
    # Renderiza la plantilla con los elementos y el cliente
...
def detalle_combo(request, combo_id):
    ...
    '''
    # Obtiene el combo por su ID o devuelve un error 404 si no existe
    combo = get_object_or_404(Combo, id=combo_id)
    imagenes = ImagenCombo.objects.filter(combo=combo)
    servicios_incluidos = combo.servicios_incluidos.all()
    
    # Captura el cliente_id desde los parámetros GET
...
def editar_perfil_cliente(request):
    ...
    # Si el método es POST, actualiza los datos del cliente
...
def cambiar_contrasena_cliente(request):
    ...
    if request.method == 'POST':
        ...
        # Verifica si la contraseña cumple con los requisitos
            ...
            # Cambia la contraseña y actualiza la sesión del usuario
