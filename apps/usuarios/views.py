...
def registro_usuario(request):
    if request.method == 'POST':
        ...
        link_activacion = request.build_absolute_uri(
            f"/usuarios/completar-registro/?{query_params}")
        ...
        try:
            ...
            messages.success(
                request, 'Registro inicial exitoso. Revisa tu correo para continuar.')
            return redirect('login_usuario')
        except (BadHeaderError, SMTPException) as e:
            # Log del error para depuración
            print(f"Error al enviar correo: {e}")
            messages.error(
                request, 'No se pudo enviar el correo. Verifica tu conexión e intenta más tarde.')
...
def activar_cuenta(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        if not user.is_active:
            ...
            login(request, user)
            messages.success(
                request, 'Cuenta activada exitosamente. Completa tu perfil.')
            return redirect('completar_registro')
        else:
            messages.warning(
                request, 'Esta cuenta ya ha sido activada anteriormente.')
...
def completar_registro(request):
    ...
    if not temp_user_data or temp_user_token != token:
        messages.error(
            request, 'El enlace de registro no es válido o ha expirado.')
        return redirect('registro_usuario')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                messages.error(
                    request, 'Ocurrió un error al completar el registro.')
        else:
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            link_reset = request.build_absolute_uri(
                reverse('resetear_contrasena', kwargs={
            ...
            messages.success(
                request, 'Hemos enviado un enlace para restablecer tu contraseña a tu correo.')
        except Usuario.DoesNotExist:
            messages.error(
                request, 'No existe un usuario registrado con este correo.')
...
def resetear_contrasena(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        if request.method == 'POST':
            ...
            user.save()
            messages.success(
                request, 'Tu contraseña se ha restablecido exitosamente.')
            return redirect('login_usuario')
...
def crear_administrador(request):
    if request.method == 'POST':
        ...
        Usuario.objects.create_user(
            username=username, email=email, password=password, rol='administrador', is_staff=True)
        messages.success(
            request, f'Administrador "{username}" creado exitosamente.')
...
def recuperar_usuario(request):
    if request.method == 'POST':
        ...
        try:
            ...
            messages.success(
                request, 'Te hemos enviado un correo con tu nombre de usuario.')
        except Usuario.DoesNotExist:
            messages.error(
                request, 'No existe un usuario registrado con ese correo.')
        return redirect('login_usuario')
